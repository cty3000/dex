...
func (c *oauthConnector) HandleCallback(s connector.Scopes, r *http.Request) (identity connector.Identity, err error) {
	...
	if userInfoResp.StatusCode != http.StatusOK {
		return identity, fmt.Errorf("OAuth Connector: failed to execute request to userinfo: userInfoResp:[%v], status:[%d]", userInfoResp, userInfoResp.StatusCode)
	}
	...
	email, err := c.addEmailFromToken(rawIDToken)
	if err != nil {
		return identity, fmt.Errorf("OAuth Connector: failed to parse id token: %v", err)
	}
	...
	// we should get email from id token
	...
	fmt.Printf("OAuth Connector: userInfoResult[%#v]\n", userInfoResult)
	fmt.Printf("OAuth Connector: Username:[%s], PreferredUsername:[%s], Email:[%s], EmailVerified:[%s]\n", identity.Username, identity.PreferredUsername, identity.Email, identity.EmailVerified)
...
func (c *oauthConnector) addEmailFromToken(token string) (string, error) {
	parts := strings.Split(token, ".")
	if len(parts) < 2 {
		return "", errors.New("invalid token")
	}
	...
	emailClaim, ok := claimsMap[c.emailKey].(string)
	if !ok {
		return "", errors.New("cannot convert to slice")
	}
